...
const TariffCard: FC<IProps> = ({ tariff, selectedTariff, setSelectedTariff }) => {
  ...
  const tariffLogo = tariff.logo ? `${MAIN_HOST}/${tariff.logo}` : 'images/cards/balanceCardDefault.svg';

  const handleCollapseClick = () => setSelectedTariff(openCollapse ? null : tariff);

  const handleSliderClick = (event: ChangeEvent<{}>, newValue: any) => setCurrentSlide(newValue);

  return (
    <div className={`${classes.container} ${openCollapse ? classes.highlightContainer : ''}`}>
      ...
      >
        <Grid container alignItems='center' wrap='nowrap' item sm={10}>
          <img src={tariffLogo} className={classes.mainImg} alt='' />
      ...
      <Collapse in={openCollapse} timeout='auto' unmountOnExit>
        <Grid container alignItems='center'>
          {(tariff?.sliders?.length ? tariff?.sliders[currentSlide]?.quotas : tariff?.quotas)?.map(quota => (
